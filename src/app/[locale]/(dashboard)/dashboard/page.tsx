...
import { Users, DollarSign, TrendingUp, Clock, ArrowUpRight, ArrowDownRight, MapPin, Lock } from 'lucide-react';
import { useUser } from '@/contexts/UserContext';

export default function DashboardPage() {
  const { localizacao, profile, temPermissao } = useUser();
      ...
      {!temLocalizacao && (
        <div className="flex flex-col items-center justify-center py-20 px-6 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-sm text-gray-500 text-center max-w-sm">
            Selecione uma empresa no seletor no topo da página para visualizar os dados do dashboard.
          </p>
        </div>
      )}

      {/* Com localização mas sem permissão de dashboard */}
      {temLocalizacao && !temAcessoDashboard && (
        <div className="flex flex-col items-center justify-center py-20 px-6 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-sm text-gray-500 text-center max-w-sm">
            Você não possui permissão para visualizar o dashboard. Entre em contato com o administrador da sua empresa.
          </p>
        </div>
      )}

      {/* Com localização e com permissão */}
      {temLocalizacao && temAcessoDashboard && (
