...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      mensagem: resultado.status === 'RENOVADO' ? 'Convite renovado e reenviado.' : 'Convite enviado com sucesso.',
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificar usuário logado via cookie de sessão
    ...
    // Se não sucesso, retornar status para o frontend tratar
    if (!resultado.sucesso) {
      return NextResponse.json(resultado, { status: 200 });
    }
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:40px 20px;">
              <tr><td align="center">
                <table width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                  ...
                  <tr>
                    <td style="padding:40px;">
                      ...
                      <p style="margin:0 0 16px;color:#6b7280;font-size:15px;line-height:1.6;">
                        ...
                        <strong style="color:#111827;">${empresa?.nome || 'empresa'}</strong> no Apprutea.
                      </p>
                      ...
                      <p style="margin:0 0 8px;color:#9ca3af;font-size:13px;">Se o botão não funcionar, copie e cole o link abaixo:</p>
                  ...
                  <tr>
                    <td style="padding:24px 40px;border-top:1px solid #f3f4f6;text-align:center;">
                      <p style="margin:0;color:#9ca3af;font-size:12px;">Se você não esperava este convite, pode ignorar este e-mail.</p>
                      <p style="margin:8px 0 0;color:#9ca3af;font-size:12px;">© ${new Date().getFullYear()} Apprutea. Todos os direitos reservados.</p>
                    </td>
                  </tr>
                </table>
              </td></tr>
